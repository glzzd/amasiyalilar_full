...
const AllBusinessPage = () => {
  const people = useMemo(() => {
    return Array.isArray(allBusiness) ? allBusiness : []
  }, [])
  ...
  const filtered = useMemo(() => {
    ...
    return people.filter((item) => {
      ...
      return !q || inName || inDesc || inPlace || inSector || inCompany
    })
  }, [query, people])
      ...
      <div className="container mx-auto px-6 pt-12">
        <div className="max-w-3xl mb-12">
          ...
          <p className="text-lg text-gray-500 leading-relaxed max-w-2xl">
            Amasiya iqtisadiyyatına töhfə verən, müxtəlif sahələrdə fəaliyyət göstərən sahibkar və biznes
            nümayəndələri.
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8 items-stretch md:items-center">
          <div className="relative flex-grow max-w-xl">
            <input
              ...
              placeholder="Axtar (ad, sektor, şirkət, yer)..."
        ...
        {filtered.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {currentItems.map((item) => (
              <Link
                to={`/amasiyalilar/business/${item.slug}`}
                key={item.id}
              ...
              >
                <div className="relative h-48 overflow-hidden bg-gray-100">
                  ...
                  <div className="absolute bottom-4 left-4 right-4">
                    ...
                    <p className="text-white/80 text-sm font-medium line-clamp-1">{item.company}</p>
                  </div>
                </div>
          ...
          <div className="text-center py-20 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
            ...
            <p className="text-gray-500 max-w-md mx-auto">
              Axtarış sorğunuza uyğun iş adamı tapılmadı. Zəhmət olmasa başqa açar sözlərlə yoxlayın.
            </p>
          </div>
        )}
