...
const IntellectualDetails = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (isMounted) setError(err.message || 'Ziyalı məlumatı yüklənərkən xəta baş verdi')
      } finally {
  ...
  const intellectual = useMemo(() => {
    return intellectuals.find(i => i.slug === slug) || null
  }, [intellectuals, slug])
  ...
  if (error) {
    return (
      <div className="container mx-auto px-4 py-20 text-center">
        ...
        <Link to="/western-azerbaijan/intellectuals" className="inline-block mt-4 px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
          Siyahıya Qayıt
        </Link>
      </div>
    )
  ...
  if (!intellectual) {
    return (
      <div className="container mx-auto px-4 py-20 text-center">
        ...
        <p className="text-gray-500 mt-2">Axtardığınız ziyalı haqqında məlumat mövcud deyil.</p>
        <Link to="/western-azerbaijan/intellectuals" className="inline-block mt-4 px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
          Siyahıya Qayıt
        </Link>
      </div>
    ...
    <div className="min-h-screen pb-16 bg-white">
      ...
      <div className="relative h-[400px] md:h-[500px] w-full">
        <div className="absolute inset-0 bg-gray-900">
           <img
            src={intellectual.image}
            alt={intellectual.name}
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="container mx-auto px-4 pb-12">
             <Link
              to="/western-azerbaijan/intellectuals"
            ...
            <div className="flex flex-wrap gap-6 text-gray-300 text-sm md:text-base">
              <div className="flex items-center gap-2">
                ...
                <span>{intellectual.birthplace}</span>
              </div>
              <div className="flex items-center gap-2">
                <Calendar size={18} className="text-blue-400" />
                <span>{intellectual.born} — {intellectual.died}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            <section>
              ...
              <div className="prose prose-lg text-gray-600 leading-relaxed max-w-none">
                <p>{intellectual.biography || intellectual.description}</p>
              </div>
            </section>

            {/* Achievements */}
            {intellectual.achievements && intellectual.achievements.length > 0 && (
              <section className="bg-blue-50 rounded-2xl p-8 border border-blue-100">
                <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
                  <Award className="text-blue-600" size={24} />
                  Nailiyyətləri və Mükafatları
                </h3>
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {intellectual.achievements.map((item, index) => (
            ...
            {/* Works */}
            {intellectual.works && intellectual.works.length > 0 && (
              <section>
                <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {intellectual.works.map((work, index) => (
                    <div key={index} className="flex items-center gap-3 p-4 rounded-xl border border-gray-200 hover:border-purple-200 hover:bg-purple-50 transition-colors group">
                      <div className="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center group-hover:bg-white transition-colors">
                        <Book size={20} className="text-gray-500 group-hover:text-purple-600" />
                      </div>
                      <span className="font-medium text-gray-700 group-hover:text-purple-900">{work}</span>
                    </div>
                  ))}
                </div>
              </section>
              ...
              <div className="space-y-6">
                {intellectual.profession && (
                  <div>
                    ...
                    <div className="text-gray-900 font-medium">{intellectual.profession}</div>
                  </div>
                )}

                {intellectual.education && intellectual.education.length > 0 && (
                  <div>
                    <div className="text-xs text-gray-500 uppercase tracking-wider font-semibold mb-2 flex items-center gap-2">
                    ...
                    <ul className="space-y-2">
                      {intellectual.education.map((edu, i) => (
                ...
                {intellectual.interesting_facts && intellectual.interesting_facts.length > 0 && (
                  <div>
                    <div className="text-xs text-gray-500 uppercase tracking-wider font-semibold mb-2 flex items-center gap-2">
                      <Lightbulb size={14} />
                      Maraqlı Faktlar
                    ...
                    <ul className="space-y-3">
                      {intellectual.interesting_facts.map((fact, i) => (
...
export default IntellectualDetails
