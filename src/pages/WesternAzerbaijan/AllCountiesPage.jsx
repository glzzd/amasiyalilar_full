...
import { Map, Landmark, Mountain, Users, CalendarDays, Ruler } from 'lucide-react'
      ...
      <div className="container mx-auto px-6">
        <div className="max-w-3xl mb-12">
          ...
          <p className="text-lg text-gray-500 leading-relaxed max-w-2xl">
            Qərbi Azərbaycanın tarixi mahalları. Memarlıq, folklor və yaşayış izləri ilə unudulmaz bölgələr.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 mb-8">
          <div className="relative max-w-xl w-full">
            <input
              ...
              placeholder="Mahal adı və ya məlumat axtar..."
              className="w-full pl-3 pr-4 py-2.5 border border-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <Landmark size={18} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400" />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {currentItems.map((item) => (
            <Link key={item.id} to={`/western-azerbaijan/city-and-districts/counties/${item.slug}`} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              ...
              <div className="p-5">
                ...
                <div className="flex items-center gap-4 text-xs text-gray-500">
                  <span className="inline-flex items-center gap-1"><Ruler size={14} /> {item.stats?.area}</span>
                  <span className="inline-flex items-center gap-1"><Users size={14} /> {item.stats?.population}</span>
                </div>
...
export default AllCountiesPage
